/*
  # Add analytics summary RPC

  - Compute analytics aggregates server-side for the Analytics page
  - Return a compact JSON payload with yearly, monthly, lead source, archive, funnel, and closing-this-month stats
  - Include filter context and optional annual GCI goal
  - Add supporting indexes for common analytics filters
*/
...
begin
  with scoped_deals as (
    select d.*,
      coalesce(d.close_date::timestamptz, d.closed_at) as close_ts,
...
create index if not exists deals_user_created_at_idx on public.deals (user_id, created_at);
create index if not exists deals_user_status_created_at_idx on public.deals (user_id, status, created_at);
create index if not exists deals_user_status_closed_at_idx on public.deals (user_id, status, closed_at);
create index if not exists deals_lead_source_id_idx on public.deals (lead_source_id);
create index if not exists deals_pipeline_status_id_idx on public.deals (pipeline_status_id);
